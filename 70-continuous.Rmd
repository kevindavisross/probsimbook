...
We have already seen Normal distributions in many previous examples.
In this section we'll explore properties of Normal distributions in a little more depth.
...
```{r, ref.label = 'normal-empirical-rule', echo=FALSE}
...
```{r, ref.label = 'standard-normal-spinner', echo=FALSE, fig.cap="(ref:cap-standard-normal-spinner)", fig.align='center', fig.show="hold", out.width="50%"}

```

Now we look more closely at where the empirical distribution and the particular bell shape of Normal distributions comes from.





A Normal density is a continuous density on the real line with a particular symmetric "bell" shape.  The heart of a Normal density is the function
\[
...
\]
Therefore, probabilities for any Normal distribution can be found by standardizing and using the Standard Normal cdf, which is basically what we do when we use the "empirical rule".
...
```{r normal-package-weights, echo=FALSE, fig.show="hold", out.width="50%"}
...
(We will see a more general result when we discuss Bivariate Normal distributions.)


```{example normal-meeting-linear-combo}
...
```{asis, fold.chunk = TRUE}
1. Let $X, Y$ be the two arrival times, so that $W=|X-Y|$ is the waiting time. We want $\text{P}(|X - Y| >15) = \text{P}(X - Y > 15) + \text{P}(X - Y < -15)$. Since $X$ and $Y$ are independent each with a Normal distribution, then $X - Y$ has a Normal distribution; the mean is $30-30 = 0$, and since $X$ and $Y$ are independent
    $$
    \text{Var}(X - Y) = \text{Var}(X) + \text{Var}(V) - 2\text{Cov}(X, Y) = 17^2 + 17^2 - 0 = 578
    $$
    So $\text{SD}(X - Y) = \sqrt{578} = 24.0$. 
    The standardized value for 15 is $(15 - 0)/24.0=0.624$; that is, 15 is 0.624 SDs above the mean, so $\IP(X - Y>15) = 0.266$. Since the distribution of $X - Y$ is symmetric about 0,
    \[
    \IP(W > 15) = \IP(|X-Y|>15) = \IP(X - Y > 15) + \IP(X - Y < - 15) = 2(0.266) = 0.53.
    \]
    On about 53% of days the first person to arrive has to wait more than 15 minutes for the second to arrive.
1. $T = \min(X, Y)$ is the time at which the first person arrives. The first person arrives before 12:15 if either $X<15$ or $Y < 15$, so $\IP(T < 15) = \IP(\{X < 15\}\cup\{Y < 15\})$. It's easier to compute the probability of the complement. Note that 15 is 0.88 SDs below the mean of $X$ ($(15 - 30)/17 = -0.88$) so $\IP(X>15) = 0.811$; similarly, $\IP(Y > 15) = 0.811$. We want
\begin{align*}
\IP(T < 15) & = 1 - \IP(X > 15, Y > 15) && \text{Complement rule}\\
& = 1 - \IP(X>15)\IP(Y > 15) && \text{independence of $X, Y$}\\
& = 1- (0.811)(0.811) & & \text{Normal distributions}\\
& = 0.342
\end{align*}
On about 34% of days the first person arrives before 12:15.
```
...
Software (`Normal(0, 1).quantile(0.985)`) shows that for a Normal distribution the 98.5th percentile is 2.17 standard deviations above the mean.
Set $100 = 76.9 + 2.17\sigma$ and solve for the standard deviation $\sigma = 10.65$ degrees F.
1. The mean is now 78.9. Now the value 100 is $(100-78.9)/10.65 = 1.98$ standard deviations above the mean.
Software (`1 - Normal(0, 1).cdf(1.98)`) shows that for a Normal distribution the probability of a value more than 1.98 standard deviations above the mean is 0.023.
...
Relative small changes in the mean can have relative large impacts on tail probabilities.
...
The previous example illustrates a common problem in statistics: comparing two (or more) groups; in particular, comparing two means.
...
```{r ref.label = 'meeting-bvn-conditional-ce', echo = FALSE, warning = FALSE, message = FALSE, cache = TRUE, fig.cap="A Bivariate Normal distribution with some conditional distributions and conditional expected values highlighted."}
...
```{theorem}
$X$ and $Y$ have a Bivariate Normal distribution if and only if every linear combination of $X$ and $Y$ has a Normal distribution. That is, $X$ and $Y$ have a Bivariate Normal distribution if and only if $aX+bY+c$ has a  Normal distribution for all^[At least one of $a$, $b$ must not be 0, unless degenerate Normal distributions are considered.  A non-random constant $c$ can be considered a *degenerate* Normal random variable with a mean of $c$ and a SD of 0.] $a$, $b$, $c$. 
```
...
Random variables $X_1, \ldots, X_n$ have a **Multivariate Normal (MVN)** (a.k.a., joint Gaussian) distribution if and only if for all non-random constants $a_1,\ldots,a_n$, the RV $a_1X_1+\cdots+a_n X_n$ has a Normal distribution. That is, if random variables have a MVN distribution then any random variable formed by taking a *linear combination* of the RVs has a Normal distribution.  In particular, each $X_i$ must have a marginal Normal distribution.
