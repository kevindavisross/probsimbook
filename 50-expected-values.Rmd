...
In this chapter we will see how characteristics based on long run averages can be defined as "expected" values.  We will some properties and some strategies for computing expected values. We will also discuss how to interpret expected values, where we'll see that the "expected" in expected value is, unfortunately, a misnomer (hence the quotes).
...
Recall the "matching problem" from Example \@ref(exm:matching-indicator). A geology museum in California has four different rocks sitting in a row on a shelf, with labels on the shelf telling what type of rock each is. An earthquake hits and the rocks all fall off the shelf and get mixed up. A janitor comes in and, wanting to clean the floor, puts the rocks back on the shelf in random order.   Let the random variable $Y$ count the number of rocks that are put back in the correct spot.
...
1. To compute the sum of the simulated values, multiply each possible value by the number of repetitions on which it occurs and then add these terms.  Then the average of the 10 values is equal to the sum of each possible value times its relative frequency
    \[
    \frac{0 \times 4 + 1 \times 3 + 2 \times 2 + 4 \times 1}{10} = 0\left(\frac{4}{10}\right)+ 1\left(\frac{3}{10}\right) + 2\left(\frac{2}{10}\right)+ 4\left(\frac{1}{10}\right)
    \]
1. Sum the 24000 values and divide by 24000. The only possible values of $Y$ are 0, 1, 2, 4 so the sum of simulated values will be of the form
    \[
    ...
    \end{align*}
    Based on the results of this simulation, the long run average value of $Y$ is approximately 0.9987.
...
1. We can continue this way, truncating to the second decimal place, then the third, and so on. Considering what happens in the limit, suggest a formula for the theoretical long run average value of $X$.
...
\end{align*}
where the sum is over values $x = 0, 1, 2, \ldots$.  Plugging in $f(x) = e^{-x}$ to the above yields 0.558. (This is a bad approximation, but the following parts refine it.  Using the left endpoint instead of the midpoint, that is, replacing $f(x)=e^{-x}$ instead of $f(x+0.5)$, yields 0.92.)
...
\end{align*}
where the sum is over values $x = 0, 0.1, 0.2, \ldots$.  Plugging in $f(x) = e^{-x}$ to the above yields 0.950. 
...
1. We can use the cdf of $X$: $\IP(X \le \E(X))=\IP(X \le 1)=F_X(1) = 1-e^{-1}$.
1. The cdf is $1-e^{-x}$, so setting the cdf to 0.5 and solving for $x$ yields the median: $0.5=1-e^{-x}$ implies $x=\log(2)\approx 0.693$.  The mean is greater than the median because the large values in the right tail "pull the average up".
...
The plots below illustrate the running average of $Y$ is  Example \@ref(exm:matching-ev).  As the number of repetitions increases, the running average of the number of matches would tend to stabilize around the expected value of 1.
...
1. $E(X)$ is not the "value that we would expect" on a \emph{single} repetition of the process; it's not even possible to observe an  $X$ of 1.5 on a single repetition. You can't flip a coin 3 times and observed 1.5 heads in the 3 flips.
...
1. Since for each outcome $X\ge Z$ we have $\E(X) \ge \E(Z)$.  If for every outcome $X$ is at least as large as $Z$, then the long run average value of $X$ should be at least as large as the long run average value of $Z$. $\E(Z)=0(1/8) + 1(4/8) + 2(2/8) + 3(1/8) = 11/8$.
...
$X$ is a discrete random variable that takes infinitely many possible values.  The sum in the expected value definition is now an infinite series.
...
So $\E(X)=2.3$.  Over many MLB games, there are 2.3 home runs hit per game on average in the long run.  Note that 2.3 is the observed average number of home runs per game in the 2018 MLB season data, displayed in Figure \@ref(fig:poisson-hr-data).
...
1. The table displays the pmf of $Z$.  
...
1. Assuming the wheel is equally likely to land on any of the 38 spaces, Guillermo wins with probability 18/38 in which case his net winnings are \$1; otherwise, his net winnings are $-1$ dollar.
...
1. Assuming the wheel is equally likely to land on any of the 38 spaces, Nadja wins with probability 1/38 in which case her net winnings are \$35; otherwise, her net winnings are $-1$ dollar.
...
1. What is the smallest *possible* value the SD of quiz scores could be?  What would need to be true about the distribution for this to happen?
1. Arrange the classes in order based on their SDs from smallest to largest.  
...
1. Both scores are above average, but Darius's is farther above average than Alfred's.  Both distributions are Normal, so the probability that an SAT score is greater than Darius's is smaller than the probabilitu that an ACT score is greater than Alfred's. That is, Darius scored relatively better. See Figure \@ref(fig:sat-z-score-plot).

```

(ref:cap-sat-z-score-plot) Comparison of the Normal distributions in Example \@ref(exm:sat-z-score). The blue mark indicated Darius's SAT score and the orange mark indicated Alfred's ACT score.
...
The idea behind Markov's inequality is that large values pull the mean up, so given a fixed value of the mean there is a limit on the probability that the random variable takes large values. The proof uses the same strategy as \@ref(exm:markov-income). Each value of $|X|$ is either at least $c$ or not.  Consider the extreme situation where each value of $|X|$ is either 0 or $c$. That is, define the random variable $Y=c\ind\{|X|\ge c\}$. Then $|X|\ge Y$; if $|X|\ge c$ then $Y=c$; if $|X|<c$ then $Y=0$.
...
- ($z = 6$.) At least 97.\% of values fall within 6 standard deviations of the mean.
...
1. $\Cov(X, Z)<0$.  If $W$ is large (roll many 1s) then $X$ (sum) tends to be small.
...
1. $\Cov(W, Z)>0$.  There is a fixed number of rolls.  If you roll lots of 4s then there must be few rolls of 1s.
...
- $\Cov(X,Y)>0$ (positive association) if above average values of $X$ tend to be associated with above average values of $Y$
- $\Cov(X,Y)<0$ (negative association) if above average values of $X$ tend to be associated with below average values of $Y$
...
The following code illustrates how to approximate covariance via simulation.  The key is to first simulate $(X, Y)$ pairs with the proper joint distribution.

```{python, eval = TRUE}

P = DiscreteUniform(0, 1) ** 2
...
1. A linear rescaling does not change the shape of the distribution or the strength of the association. However, a linear rescaling does relabel the axis and change measurements.  A covariance of 96 inches$\times$pounds corresponds to a covariance of 8 feet$\times$pounds.
